...
class LibraryReaderController extends Controller
{
    public function index()
    {
        ...
        $LibraryReader = DB::table('user AS u')
            ...
            ->select('c.COMIC_ID', 'c.COMIC_TITLE', 'c.COMIC_GENRE', 'c.COMIC_COVER', 'subq.USER_NAME', 'lr.LIBRARY_READER_ID')
            ->where('lr.LIBRARY_READER_ID', '=', $LibraryReaderId)
            ->whereNotNull('c.COMIC_TITLE')
            ->get();

        return Inertia::render('Application/LibraryReader', [
            'LibraryReader' => $LibraryReader,
            'CombinedData' => $CombinedData,
            'Chapter' => $Chapter,
            'activePage' => 'libraryreader',
        ]);
    }
}